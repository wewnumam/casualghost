...
public class ClearBuilding : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D collider) {
        ...
        if (collider.gameObject.CompareTag(Tags.BULLET_TYPE_ONE) || collider.gameObject.CompareTag(Tags.BULLET_TYPE_TWO) || collider.gameObject.CompareTag(Tags.ENEMY)) {
            if (collider.gameObject.CompareTag(Tags.ENEMY)) {
                if (collider.gameObject.GetComponent<Enemy>().IsEnemyTypeBoss()) {
                    Destroy(gameObject);
                    return;
                }
            }
